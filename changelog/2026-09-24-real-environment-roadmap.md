...
## 追加：M4/M5 无真实环境时的可开发范围（2026-09-24 澄清）

- 结论：真实环境阻塞的是里程碑"退出验收"，不是全部代码开发。
- M4 现在可做（纯逻辑，Fake 闭环可验证）：任务超时、有限重试（非幂等动作不自动
  重试）、取消完善与中断恢复、Control Server/Desktop Agent 双重策略校验、日志脱敏、
  截图/trace 保留与自动清理策略、禁止动作拦截补充。
- M4 必须等真实环境：前台窗口双重检查的 FlaUI 实现（接口可先定义）、两秒紧急停止
  实测、八小时稳定性测试、页面变化诊断（依赖真实适配器）。
- M5 现在可做：离线演示夹具与降级演示、管理台完整功能、手册框架、成功率/耗时统计
  模块代码；真实数据填充、Windows 全链路 E2E、版本冻结和备份视频必须等待。
