...
| GitHub Actions | NOT_EXECUTED | 待推送并创建 PR 后执行。 |
...
- 结果：本地干净完整质量门已通过，待 UI 与 GitHub Actions 验证。

## 风险与限制

- macOS 本地不能直接复现 Windows checkout，但可验证 Git 属性和 .NET 格式检查；
  最终 Windows 结果以 GitHub Actions 为准。
